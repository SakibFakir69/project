/**
 * playwright.extractor.ts — v2 Optimized (Object Options Signature)
 * ─────────────────────────────────────────────────────────────────────────────
 * CRITICAL FIXES:
 * ✅ Fixed signature mismatch error by accepting an options object
 * ✅ Wired up proxy option to securely apply rotation configuration to contexts
...
export async function extractWithPlaywright(options: PlaywrightExtractorOptions): Promise<PlaywrightResult> {
  const { url, platform, proxy, signal } = options;
  ...
  try {
    ...
    log("info", "Creating browser context", { url: url.slice(0, 80), platform, hasProxy: !!proxy });
    ...
    // Merged user config context parameters cleanly with proxy routing properties
    context = await browser.newContext({
      ...
      proxy:              proxy ?? undefined, // Applied rotation arguments safely here!
      extraHTTPHeaders: {
