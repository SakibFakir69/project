...
class MemoryCache {
    ...
    async delete(key) { if (this.map.delete(key))
        this._size--; }
...
class RedisCache {
    ...
    size() { return this.client ? -1 : this.mem.size(); }
}
let cache;
if (REDIS_URL) {
...
async function checkYtDlpFreshness() {
    try {
        ...
        const { mtimeMs } = statSync(stdout.trim());
...
async function resolveRedirectUrl(url) {
    try {
        ...
        if (resolved !== url)
            log("debug", "resolver", "Resolved redirect", { from: url.slice(0, 60), to: resolved.slice(0, 60) });
        return resolved;
    }
    catch {
        return url;
...
function buildAttemptContext(url, platform, attemptIndex, signal) {
    const proxy = proxyPool.pickByIndex(attemptIndex);
    const cookie = cookieManager.byIndex(platform, attemptIndex);
...
function buildYtDlpArgs(url, platform, ctx, extra) {
    ...
    const args = [
        ...
        ...proxyPool.ytdlpArgs({ url: ctx.proxy }),
...
function buildGalleryDlArgs(url, platform, ctx, extra) {
    const adapter = getAdapter(platform);
...
async function getCobaltDownloadUrl(resolvedUrl, quality = "720", signal) {
    if (!cobaltReachable)
        throw new Error("[Cobalt] Unreachable");
    if (cbIsOpen("cobalt"))
        throw new Error("[CB] Cobalt circuit open");
    ...
    switch (data.status) {
        ...
        case "picker": {
            ...
            const aud = data.picker?.find(p => p.type === "audio") ?? null;
            cbSuccess("cobalt");
...
async function getYtDlpWithMutation(url, platform, quality, signal) {
    if (cbIsOpen("ytdlp"))
        throw new Error("[CB] yt-dlp circuit open");
    const adapter = getAdapter(platform);
    ...
    for (let attempt = 0; attempt < adapter.maxAttempts; attempt++) {
        ...
        try {
            const { stdout } = await withSemaphore(async () => {
                ...
                try {
                    return await execFilePromise("yt-dlp", args, { ...EXEC_OPTS, signal: ac.signal });
                }
                finally {
                    clearTimeout(tid);
                }
            ...
            try {
                ...
                throw new Error("yt-dlp: non-JSON output");
            }
            let videoUrl = null;
            let audioUrl = null;
            ...
            if (!videoUrl)
                throw new Error("yt-dlp: no video URL in output");
            // Mark proxy and cookie as successful
            const proxy = proxyPool.pickByIndex(attempt);
            if (proxy)
                proxyPool.success(proxy);
            if (ctx.cookiePath)
                cookieManager.markSuccess(platform, ctx.cookiePath);
            cbSuccess("ytdlp");
            ...
            const proxy = proxyPool.pickByIndex(attempt);
            if (proxy)
                proxyPool.failure(proxy, platform, c.category === "rate_limit");
            if (ctx.cookiePath)
                cookieManager.markFailed(platform, ctx.cookiePath);
...
async function getYtDlpInfo(url, platform) {
    ...
    const { stdout } = await withSemaphore(() => execFilePromise("yt-dlp", args, { ...EXEC_OPTS, timeout: TIMEOUTS.info ?? 60_000 }));
    const lines = stdout.trim().split("\n").filter(Boolean);
    let meta;
    try {
        meta = JSON.parse(lines[lines.length - 1]);
    ...
    if (!meta?.title)
        throw new Error("Video not found via yt-dlp");
    return { title: meta.title, thumbnail: meta.thumbnail ?? null, duration: meta.duration ?? 0, ext: meta.ext ?? "mp4" };
}
async function getGalleryDlDownloadUrl(url, platform, signal) {
    if (cbIsOpen("gallerydl"))
        throw new Error("[CB] gallery-dl circuit open");
    ...
    const { stdout } = await withSemaphore(async () => {
        ...
        if (signal)
            signal.addEventListener("abort", () => ac.abort(), { once: true });
        try {
            return await execFilePromise("gallery-dl", args, { ...EXEC_OPTS, signal: ac.signal });
        }
        finally {
            clearTimeout(tid);
        }
    ...
    if (!stdout.trim())
        throw new Error("gallery-dl: empty output");
    const entries = [];
    ...
    if (!entries.length)
        throw new Error("gallery-dl: no parseable JSON");
    const videoEntry = entries.find(e => {
        const m = e[1];
        if (typeof m === "string")
            return /\.(mp4|webm|mov|mkv|avi|flv|ts)(\?|$)/i.test(m);
        return m?.extension && /mp4|webm|mov|mkv|avi|flv|ts/i.test(m.extension);
    }) ?? entries[0];
    const meta = videoEntry[1];
    let directUrl, filename, title, thumbnail = null;
    ...
    if (!directUrl)
        throw new Error("gallery-dl: no direct URL");
    cbSuccess("gallerydl");
...
function guessMime(filename) {
    ...
    return { mp4: "video/mp4", webm: "video/webm", mkv: "video/x-matroska", mov: "video/quicktime", m4a: "audio/mp4", aac: "audio/aac", mp3: "audio/mpeg", ogg: "audio/ogg", ts: "video/mp2t", flv: "video/x-flv" }[ext] ?? "application/octet-stream";
}
// ─────────────────────────────────────────────────────────────────────────────
// 1. GET VIDEO INFO
// ─────────────────────────────────────────────────────────────────────────────
export const getVideoInfo = async (req, reply) => {
    ...
    const result = await dedupe(cacheKey, async () => {
        ...
        return data ? { source: "ytdlp", data } : null;
    });
    if (result) {
...
export const getDownloadLink = async (req, reply) => {
    ...
    const ac = new AbortController();
    req.raw.on("close", () => { if (!reply.sent)
        ac.abort(); });
    ...
    const execute = async () => {
        ...
        if (adapter.useCobalt && !cbIsOpen("cobalt") && cobaltReachable) {
            try {
                ...
                log("warn", "cobalt", "Failed", { error: e.message?.slice(0, 100) });
            }
        }
        ...
        if (!cbIsOpen("ytdlp")) {
            try {
                ...
                log("warn", "ytdlp", "All attempts failed", { error: e.message?.slice(0, 100) });
            }
        }
        // ── TIER 3: gallery-dl ───────────────────────────────────────────────────
...
// 3. RESOLVE URL
// ─────────────────────────────────────────────────────────────────────────────
export const resolveUrl = async (req, reply) => {
    const { url } = req.query;
    if (!url)
...
// ─────────────────────────────────────────────────────────────────────────────
export const tunnel = async (req, reply) => {
    ...
    try {
        ...
        if (cl)
            reply.header("Content-Length", cl);
        if (cr)
            reply.header("Content-Range", cr);
...
// 5. HEALTH CHECK
// ─────────────────────────────────────────────────────────────────────────────
export const healthCheck = async (_req, reply) => {
    const cobaltOk = await fetch(`${COBALT_URL}/`, { method: "HEAD", signal: AbortSignal.timeout(3_000) })
        .then(() => true).catch(() => false);
    let ytdlpAgeDays = null;
    try {
        const { stdout } = await execFilePromise("which", ["yt-dlp"], { encoding: "utf8" });
        ytdlpAgeDays = parseFloat(((Date.now() - statSync(stdout.trim()).mtimeMs) / 86_400_000).toFixed(1));
    }
    ...
    return reply.code(200).send({
        ...
        circuits: Object.fromEntries(Object.entries(circuitBreakers).map(([k, v]) => [k, { open: v.openUntil > Date.now(), failures: v.failures }])),
